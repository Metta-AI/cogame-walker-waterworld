...
function stageNote() {
  ...
  try {
    var length = Module._waterworld_stage_len ? Module._waterworld_stage_len() : 0;
    if (!length) return '';
    var pointer = Module._waterworld_stage_ptr();
...
function runtimeError() {
  var length = Module._waterworld_error_len();
  if (!length) {
    var stage = stageNote();
    return stage
  ...
  var pointer = Module._waterworld_error_ptr();
  return new TextDecoder().decode(
    Module.HEAPU8.slice(pointer, pointer + length));
}
...
function ingestPacket() {
  var length = Module._waterworld_packet_len();
  if (!length) throw new Error('Replay runtime produced an empty frame');
  var pointer = Module._waterworld_packet_ptr();
...
function sendRuntimeInput(bytes) {
  ...
  copyIntoRuntime(bytes, function (pointer, length) {
    Module._waterworld_input(pointer, length);
  });
}
...
async function start() {
  ...
  try {
    ...
    var loaded = copyIntoRuntime(bytes, function (pointer, length) {
      return Module._waterworld_load_replay(pointer, length);
    });
    ...
    postMessage({
      type: 'loaded',
      mismatchTick: Module._waterworld_mismatch_tick()
    });
  } catch (error) {
    reportFailure(error);
  }
...
function applyInputNow() {
  ...
  try {
    if (Module._waterworld_frame() < 0) throw new Error(runtimeError());
    ingestPacket();
    postMessage({
      type: 'inputApplied',
      mismatchTick: Module._waterworld_mismatch_tick()
...
importScripts('./wire_constants.js', './broadcast_core.js', './waterworld_replay.js');
