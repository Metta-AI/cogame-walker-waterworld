...
var tank = seatedSim()
var engine = initDecisionEngine(tank)
for seat in 0 ..< SkimmerCount:
  engine.seats[seat].isLlm = true
  engine.seats[seat].prompt = "operator guidance for seat " & $seat
  tank.seatNames[seat] = "REAL-POLICY-NAME-" & $seat
var rng = initRand(8080)

for trial in 0 ..< 200:
  for i in 0 ..< SkimmerCount:
    tank.skimmers[i].x = SkimmerRadius +
      int32(rng.next() mod uint64(ArenaW - 2 * SkimmerRadius))
    tank.skimmers[i].y = SkimmerRadius +
      int32(rng.next() mod uint64(ArenaH - 2 * SkimmerRadius))
  for f in 0 ..< FoodCount:
    tank.food[f].state = psLive
    tank.food[f].x = FoodRadius + int32(rng.next() mod uint64(ArenaW - 2 * FoodRadius))
    tank.food[f].y = FoodRadius + int32(rng.next() mod uint64(ArenaH - 2 * FoodRadius))
  for q in 0 ..< PoisonCount:
    tank.poison[q].state = psLive
    tank.poison[q].x = PoisonRadius +
      int32(rng.next() mod uint64(ArenaW - 2 * PoisonRadius))
    tank.poison[q].y = PoisonRadius +
      int32(rng.next() mod uint64(ArenaH - 2 * PoisonRadius))
  for seat in 0 ..< SkimmerCount:
    let
      skimmer = tank.skimmerForSeat(seat)
      me = tank.skimmers[skimmer]
      frame = tank.frameFor(skimmer)
      view = engine.seatViewJson(tank, seat, 9, frame)
      message = userMessage(engine.seats[seat].prompt, view)
    # A particle appears IFF its centre is within the sensor range.
    for f in 0 ..< FoodCount:
      let
        near = withinUm(tank.food[f].x, tank.food[f].y, me.x, me.y, SensorRange)
    ...
    for q in 0 ..< PoisonCount:
      let
        near = withinUm(tank.poison[q].x, tank.poison[q].y, me.x, me.y, SensorRange)
...
block structuralLimits:
  ...
  check("the controller never reads tank.food directly",
    "tank.food" notin source, "tank.food")
  check("the controller never reads tank.poison directly",
    "tank.poison" notin source, "tank.poison")
  check("the controller never reads perm", "tank.perm" notin source)
  check("the controller never reads the RNG", "tank.rng" notin source)
  ...
  check("the sensor frame never reads perm", "tank.perm" notin sensorSource)
